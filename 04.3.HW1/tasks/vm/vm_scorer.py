...
OPERATION_LEVELS = {
    ...
    "LOAD_FROM_DICT_OR_DEREF": 3,
    "LOAD_FROM_DICT_OR_GLOBALS": 3,
    "LOAD_GLOBAL": 1,
    "LOAD_LOCALS": 3,
    "LOAD_NAME": 1,
    "LOAD_SUPER_ATTR": 3,
...
def dump_tests_stat(stream: tp.TextIO, scorer: Scorer) -> None:
    ...
    data = [
        ...
        "\n".join("\t{}: {}".format(operation, count) for operation, count in sorted(scorer.get_total_stats().items())),
        "Tests levels distribution:",
        "\n".join("\t{}: {}".format(level, count) for level, count in scorer.get_levels_stats().items()),
        "Operations coverage:",
        "\t" + "{}/{}".format(scorer.get_operations_coverage(), scorer.get_operations_count()),
        "Operations by levels coverage:",
        "\n".join(
            "\t{}: {}/{}".format(level, count, scorer.get_level_operations_count()[level])
